...
const InventoryModel = require("./Models/InventoryComponent");

mongoose
  .connect("mongodb://localhost:27017/IMS-Project")
...
app.get("/dashboard", async (req, res) => {
  let existingItems = await InventoryModel.find();
...
app.post("/updateItem", async (req, res) => {
  ...
  if (Is_Job_Work && (!Job_Work_Name || !Job_Work_GST || !Job_Work_Address)) {
    return res.json({ message: "Enter All Job Work Fields" });
  }
  ...
  await InventoryModel.findByIdAndUpdate(editItemId, {
    Item_Name,
    Challan_No: Challan_Number,
    Category,
    Sub_Category,
