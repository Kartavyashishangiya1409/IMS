...
const Items = () => {
      ...
      <div className="item-list">
        <div className="item-card-grid">
          {allItems.map((item, index) => (
            <div className={`item-card ${item.Is_Job_Work ? "job-card" : ""}`} key={item._id}>
  ...
  <div className="item-meta">
    <p><strong>Challan:</strong> {item.Challan_No}</p>
    <p><strong>GST:</strong> {item.GST_No}</p>
  </div>

  <div className="item-info">
    <p><strong>Category:</strong> {item.Category}</p>
    <p><strong>Sub:</strong> {item.Sub_Category || "N/A"}</p>
    <p><strong>HSN:</strong> {item.HSN_Code}</p>
    <p><strong>Attr:</strong> {item.Attribute}</p>
  </div>

  <div className="item-qty">
    <span>Gross: {item.Gross_QTY}</span>
    <span>Tare: {item.Tare_Weight}%</span>
    <span>Net: {item.Net_QTY}</span>
  </div>

  <div className="item-price">₹ {Number(item.Total_Price).toFixed(2)}</div>

  <div className="job-flag">
    Job Work: 
    <span className={item.Is_Job_Work ? "yes" : "no"}>
      {item.Is_Job_Work ? "YES" : "NO"}
    </span>
  </div>

  {item.Is_Job_Work && (
    <div className="job-panel">
      <p><strong>{item.Job_Work_Name}</strong></p>
      <p>GST: {item.Job_Work_GST}</p>
      <p>{item.Job_Work_Address}</p>
    </div>
  )}

  <div className="item-actions">
    <button className="edit-btn" onClick={editflag ? handleUpdateItem : handleEditItem}>Edit</button>
