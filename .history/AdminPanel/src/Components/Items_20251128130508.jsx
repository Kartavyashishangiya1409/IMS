        ...
        <div className="item-card-grid">
          {allItems.map((item, index) => (
            <div className="item-card" key={item._id}>
              ...
              <h5>Tare: {item.Tare_Weight}%</h5>
              <h5>Net: {item.Net_QTY}</h5>
              <h5>₹ {Number(item.Total_Price).toFixed(2)}</h5>

              <h5>Job Work: {item.Is_Job_Work ? "Yes" : "No"}</h5>

{item.Is_Job_Work && (
  <div className="jobwork-details">
    <h5>Job Name: {item.Job_Work_Name}</h5>
    <h5>Job GST: {item.Job_Work_GST}</h5>
    <h5>Job Address: {item.Job_Work_Address}</h5>
  </div>
)}
